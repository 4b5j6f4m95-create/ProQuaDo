...
**Was stattdessen begrenzt: die Zahl der Transaktionen je Stapel.** An Postgres' eigenem Zähler gemessen (`pg_stat_database.xact_commit + xact_rollback` vor und nach Szenario 2, 200 Stapel): **4524 Transaktionen, also 22,6 je Stapel und 5,7 je Kommando.** Der Grund steht in `sync-commands.ts`: jedes Kommando durchläuft `claimCommand`, `executeCommand`, `finalizeCommand` und `rememberBatchVersion` als **je eigene** Transaktion, jede mit `BEGIN`, `set_config` für den Mandantenkontext und `COMMIT`. `rememberBatchVersion` öffnet eine ganze Transaktion, um eine einzige Spalte zu lesen.
...
1. **Transaktionen je Kommando zusammenlegen** (5,7 → 3 wäre realistisch, `rememberBatchVersion` gehört in `finalizeCommand` hinein). Der größte Hebel, aber echte Programmierarbeit an einem Pfad mit Konfliktsemantik — die Reihenfolge Claim → Ausführung → Quittung ist Absicht (siehe `PENDING` oben), das Zusammenlegen darf sie nicht aufheben.
...
3. **Zähler je Produktionsauftrag statt je Organisation.** +15 %, aber viel Umbau; `outbox-sequence.ts` benennt genau diesen Weg als den vorgesehenen.
