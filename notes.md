...
| ------------------------------- | ------------ | ------------------------------ |
| Schichtwechsel-Sync, 200 Geräte | p95 < 3 s    | **3,0–3,1 s** (ein Lauf 4,7 s) |
| Deadlocks                       | 0            | 0                              |
...
- **Einen PR erst mergen, wenn die Checks grün sind — und zwar mit `scripts/merge-when-green.sh <nr>`, nicht aus dem Gedächtnis.** `gh pr merge --auto` klingt nach Warten, wartet aber nur, wenn das Repository erforderliche Statuschecks kennt; Branch Protection und Rulesets sind bei GitHub für private Repositories dem Pro-Plan vorbehalten, hier gibt es also keine serverseitige Bremse. PR #3 ist genau deshalb vor seinen eigenen Checks gelandet — ausgerechnet der PR, dessen Beleg ein Test war. Sobald das Repository öffentlich ist oder auf Pro läuft, gehört die Regel nach GitHub und das Skript weg.
